...
function typeSymbol(unit: Unit.Atomic, index: StructureElement.UnitIndex) {
...
function interBondCount(structure: Structure, unit: Unit.Atomic, index: StructureElement.UnitIndex): number {
    return structure.links.getBondIndices(index, unit).length
}
...
function eachInterBondedAtom(structure: Structure, unit: Unit.Atomic, index: StructureElement.UnitIndex, cb: (unit: Unit.Atomic, index: StructureElement.UnitIndex) => void): void {
    // inter
    const interIndices = structure.links.getBondIndices(index, unit)
    for (let i = 0, il = interIndices.length; i < il; ++i) {
        const b = structure.links.bonds[i]
        cb(b.unitB, b.indexB)
    }
}
...
function bondToElementCount(structure: Structure, unit: Unit.Atomic, index: StructureElement.UnitIndex, element: Elements): number {
    let count = 0
