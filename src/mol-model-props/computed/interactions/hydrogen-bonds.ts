...
import { ComputedValenceModel } from '../valence-model';
...
function getUnitValenceModel(structure: Structure, unit: Unit.Atomic) {
    const valenceModel = ComputedValenceModel.get(structure)
    if (!valenceModel) throw Error('expected valence model to be available')
    const unitValenceModel = valenceModel.map.get(unit.id)
...
export function addHydrogenBonds(structure: Structure, unit: Unit.Atomic, features: Features, builder: InteractionsBuilder, props: HydrogenBondsProps) {
    ...
    const valenceModel = ComputedValenceModel.get(structure)
    if (!valenceModel || !valenceModel.map.has(unit.id)) throw new Error('valence model required')

    const { idealGeometry } = valenceModel.map.get(unit.id)!
